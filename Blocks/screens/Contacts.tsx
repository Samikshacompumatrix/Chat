...
const ContactsScreen = ({ user, navigation }) => {
...
async function  showCameraPicker  () 
{
  ...
  const renderUser = ({ item }) => (
    <TouchableOpacity onPress={() => navigation.navigate('singlechats', { name: item.name, uid: item.uid,img:item.image })}>
  ...
  return (
    <View style={styles.safeArea}>
      ...
      <View style={styles.area1}>
        ...
        <Image source={{uri:currentUser?.profile_img}} 
         style={styles.profileview} resizeMode='cover' />
        <Text style={{ ...styles.nameText, marginHorizontal: 20, fontSize: 20, marginTop: 10, color: 'black' }}>Contacts</Text>
...
area1:{
  flexDirection: 'row', justifyContent: 'space-between', marginHorizontal: 10, marginTop: 30
},
profileview:{
  width: 50,
  height: 50,
